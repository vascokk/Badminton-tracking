...
game_list = ['match1','match2','match3','match4', 'match5', 'match6', 'match7', 'match8', 'match9', 'match10', 'match11', 'match12', 'match13', 'match14', 'match15', 'match16', 'match17', 'match18']
p = os.path.join(TRAIN_DATA, game_list[0], 'frame', '1_01_02', '1.png')
...
for game in game_list:
	all_path = glob(os.path.join(TRAIN_DATA,game, 'frame', '*'))
	...
	for p in train_path:
		...
		for i in range(num):
			unit = []
			# Change to load 3 frames every iter
			for j in range(3):
				temp = len(no)-1
				if (i+j >= len(no)-1):
					target = str(no[len(no)-1]) + '.png'
				else:
			...
			for j in range(3):
				...
				if v[idx] == 0:
					...
					unit.append(genHeatMap(WIDTH, HEIGHT, int(x[idx]/ratio), int(y[idx]/ratio), sigma, mag))
			y_data_tmp.append(unit) 
			del unit

		x_data_tmp2 = np.asarray(x_data_tmp)
		del x_data_tmp
		x_data = x_data_tmp2.astype('float64')
		del x_data_tmp2
		x_data=(x_data/255)
